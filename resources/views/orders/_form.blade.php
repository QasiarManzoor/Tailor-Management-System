<div class="row g-4">
    <div class="col-md-4">
        ...
        <select name="customer_id" id="customer_id" class="form-select @error('customer_id') is-invalid @enderror" required>
            ...
            @foreach ($customers as $customerOption)
                <option value="{{ $customerOption->id }}" @selected((int) old('customer_id', $order->customer_id) === $customerOption->id)>{{ $customerOption->name }} - {{ $customerOption->phone }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-4">
        ...
        <div class="form-text">{{ __('tailor.measurements.help.saved_measurement') }} {{ \Illuminate\Support\Facades\Lang::get('tailor.measurements.help.saved_measurement', [], 'ur') }}</div>
        @include('partials.field-error', ['field' => 'measurement_id'])
    </div>
    <div class="col-md-4">
        <label class="form-label" for="order_type">Order Type</label>
        <input type="text" id="order_type" name="order_type" value="{{ old('order_type', $order->order_type) }}" class="form-control @error('order_type') is-invalid @enderror" placeholder="Example: 2 x Shalwar Kameez" required>
        @include('partials.field-error', ['field' => 'order_type'])
    </div>

    <div class="col-md-3">
        <label class="form-label" for="quantity">Quantity</label>
        <input type="number" min="1" id="quantity" name="quantity" value="{{ old('quantity', $order->quantity ?: 1) }}" class="form-control @error('quantity') is-invalid @enderror" required>
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" min="0" id="total_amount" name="total_amount" value="{{ old('total_amount', $order->total_amount) }}" class="form-control @error('total_amount') is-invalid @enderror" required>
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" min="0" id="advance_amount" name="advance_amount" value="{{ old('advance_amount', $order->advance_amount) }}" class="form-control @error('advance_amount') is-invalid @enderror" required>
    ...
    <div class="col-md-4">
        ...
        <input type="text" id="balance_preview" class="form-control" value="Rs. 0.00" disabled>
    </div>

    <div class="col-md-4">
        @include('partials.bilingual-text', ['key' => 'tailor.orders.booking_date', 'for' => 'booking_date', 'tag' => 'label', 'class' => 'form-label'])
        <input type="date" id="booking_date" name="booking_date" value="{{ old('booking_date', optional($order->booking_date)->format('Y-m-d') ?: $order->booking_date) }}" class="form-control @error('booking_date') is-invalid @enderror" required>
    ...
    <div class="col-md-8">
        ...
        <textarea id="fabric_details" name="fabric_details" rows="3" class="form-control @error('fabric_details') is-invalid @enderror" placeholder="Fabric type, color, supplied by customer or shop, lining notes">{{ old('fabric_details', $order->fabric_details) }}</textarea>
    ...
    <div class="col-12">
        ...
        <textarea id="special_instructions" name="special_instructions" rows="4" class="form-control @error('special_instructions') is-invalid @enderror" placeholder="Neck design, cuff notes, urgency, delivery reminders">{{ old('special_instructions', $order->special_instructions) }}</textarea>
...
<script>
    ...
    function syncBalance() {
        ...
        balanceField.value = `Rs. ${balance.toFixed(2)}`;
    }
