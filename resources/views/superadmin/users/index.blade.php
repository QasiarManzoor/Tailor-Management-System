...
@section('page-subtitle', 'Create, update, activate, and review application users by shop.')
        ...
        <form method="GET" action="{{ route('superadmin.users.index') }}" class="card-soft p-3">
            <div class="row g-3 align-items-end">
                <div class="col-md-8">
                    ...
                    <input type="text" class="form-control" id="search" name="search" value="{{ $search }}" placeholder="Name, email, role, or shop">
                </div>
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            <tr>
                                ...
                                <td>{{ $user->email }}</td>
                                <td>{{ $user->shop?->name ?? 'Unassigned' }}</td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end gap-2 flex-wrap">
                                        @if ($user->shop)
                                            <form method="POST" action="{{ route('superadmin.shops.manage', $user->shop) }}">
                                                @csrf
            ...
            <div class="mobile-record-grid gap-2">
                @foreach ($users as $user)
                    <article class="record-card">
                        ...
                        <div class="record-meta">{{ $user->shop?->name ?? 'Unassigned' }}</div>
